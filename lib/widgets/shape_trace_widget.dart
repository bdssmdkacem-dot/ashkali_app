...
class _ShapeTraceWidgetState extends State<ShapeTraceWidget> {
  ...
  void _onPanUpdate(DragUpdateDetails details) {
    // Must convert against the canvas's own RenderBox (via _repaintKey),
    // not context.findRenderObject() - that returned the RenderObject of
    // the whole Column (title + canvas + button), so every point landed
    // offset by however tall the title/spacing above the canvas was.
    // From the child's finger, the stroke looked completely disconnected
    // from where they were touching - effectively "not registering".
    final box = _repaintKey.currentContext?.findRenderObject() as RenderBox?;
    if (box == null) return;
    final localPosition = box.globalToLocal(details.globalPosition);
    // Ignore drag points outside the canvas bounds (e.g. finger sliding
    // past the edge) so stray points don't skew the coverage score.
    if (localPosition.dx < 0 || localPosition.dy < 0 ||
        localPosition.dx > widget.canvasSize || localPosition.dy > widget.canvasSize) {
      return;
    }
    setState(() {
      _userPoints.add(localPosition);
    });
  }

  void _onPanStart(DragStartDetails details) {
    if (_evaluated) return; // wait for explicit reset before a new attempt
    final box = _repaintKey.currentContext?.findRenderObject() as RenderBox?;
    if (box == null) return;
    setState(() {
